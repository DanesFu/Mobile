...
class Message2 extends Component {
  ...
  componentDidMount() {
    ...
    // var fujiang=this.props.match.url;
    // var address2={add:this.props.match.url}
    // sessionStorage.setItem('address2', JSON.stringify(address2));
    // const title =  this.props.match.params.title;
    // console.log(title)
    var info={message1:'datatwo'};
    ...
    axios.get('http://www.hzwotu.com/ApiTest/policyView?id=1')
      .then(function (res) {
        _this.setState({
  ...
  // var dataForm2 ='policy_id=1',dataForm;
  if(dataForm==='question='){
    Toast.info('问题不能为空', 1);
     console.log('kong');
  }else{
    $.ajax({
    url: 'http://www.hzwotu.com/ApiTest/faqAsk?id=1', 
    type: 'POST',
    data: dataForm,
    cache: false,
    contentType: false,
    ...
    return (
      <div className='messageTab'>
            ...
            <div>
                <Tabs tabs={tabs}
                  ...
                  swipeable={false}
                   >
                    <div className='tab'>
                        <img src={dataImage} style={{ marginTop: 10 }} alt='' />
                        <p className='my-p'></p>
        ...
        <div className="home-apply">
           <Link to={{ pathname: '/Approve/'+dataTitle+'' }} className='primary-button'>
                <span>快速申请</span>
          </Link>
        </div>
      </div>
