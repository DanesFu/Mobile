...
class Message2 extends Component {
  ...
  handleClick(){
    const info={message1:'FreeSponsorship'};
    sessionStorage.setItem('key', JSON.stringify(info));
  }
  handleClick2(){
    const info={message1:'EntrepreneurshipBest'};
    sessionStorage.setItem('key', JSON.stringify(info));
  }
  componentDidMount() {
    ...
    axios.get('http://www.hzwotu.com/ApiTest/policyView?id=4')
      .then(function (res) {
        _this.setState({
  ...
  // var dataForm2 ='policy_id=5',dataForm;
  if(dataForm==='question='){
    Toast.info('问题不能为空', 1);
     console.log('kong');
  }else{
    $.ajax({
    url: 'http://www.hzwotu.com/ApiTest/faqAsk?id=5', 
    type: 'POST',
    data: dataForm,
    cache: false,
    contentType: false,
  ...
  renderFaqList() {
    ...
    return (
      <div className='messageTab'>
        ...
        <div className="home-apply2">
          <Link to={{ pathname: '/Approve/'+dataTitle+'' }} className='primary-button2' onClick={this.handleClick}>
                <span>无偿资助</span>
          </Link>
          <Link to={{ pathname: '/Approve/'+dataTitle+'' }} className='primary-button2' onClick={this.handleClick2}>
                <span>企业扶优</span>
          </Link>
        </div>
      </div>
